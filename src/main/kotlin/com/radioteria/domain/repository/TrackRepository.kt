...
interface TrackRepository : Repository<Track, Long> {
    ...
    @Query("SELECT t.offset + t.duration FROM Track t WHERE t.channelId = ?1 ORDER BY t.offset DESC")
    fun getTracklistDurationByChannelId(id: Long?): Long

    @Query("FROM Track t WHERE t.offset >= ?2 AND t.offset + t.duration > ?2 AND t.channelId = ?1 ORDER BY t.offset DESC")
    fun findOneByChannelIdAndLapTime(channelId: Long?, LapTime: Long): Track?

    @Modifying
    @Query("UPDATE Track t SET t.offset = t.offset + ?2 WHERE t.channelId = ?1 AND t.offset > ?3 ORDER BY t.offset DESC")
    fun moveOffsetsByAmountAfterGiven(channelId: Long?, amount: Long, offset: Long)

    @Modifying
    @Query("UPDATE Track t SET t.offset = t.offset + ?2 WHERE t.channelId = ?1 AND ?3 BETWEEN ?3 AND ?4 ORDER BY t.offset DESC")
